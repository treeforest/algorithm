...
{
public:
	SeqList(int initlen = LIST_INIT_SIZE);
	~SeqList();
...
template<typename ElemType>
SeqList<ElemType>::SeqList(int initlen)
{
...
template<typename ElemType>
bool SeqList<ElemType>::Get(int i, ElemType& e)
{
	if (i < 0 || i >= m_len) {
		return false;
	}
...
template<typename ElemType>
int SeqList<ElemType>::Locate(ElemType e, bool(*compare)(ElemType, ElemType))
{
	int i;
...
template<typename ElemType>
bool SeqList<ElemType>::Prior(ElemType cur, ElemType& pre, bool(*compare)(ElemType, ElemType))
{
...
template<typename ElemType>
bool SeqList<ElemType>::Next(ElemType cur, ElemType& next, bool(*compare)(ElemType, ElemType))
{
	int i = Locate(cur, compare);
	if (i < 0 || i >= m_len - 1) {
		return false;
...
template<typename ElemType>
bool SeqList<ElemType>::Insert(int i, const ElemType e)
{
	if (i < 0 || i > m_len)
		return false;
...
template<typename ElemType>
bool SeqList<ElemType>::Delete(int i, ElemType& e)
{
	if (i < 0 || i >= m_len) {
		return false;
	}
